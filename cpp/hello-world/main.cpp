// Mirrors rust/hello-world: inserts an
// <h1 class="hello from-wasm">Hello from Wasm!</h1> into the page.
//
// Built as a WebAssembly component in C++ using the wit-bindgen C bindings
// generated from wit/world.wit (the `c` generator targets C and C++). The DOM
// is reached entirely through the component's imported resources, exactly like
// the Rust version — no JS glue beyond the shared host in web/host.
...
void exports_hello_world_run(void) {
  ...
  hello_world_own_document_t document =
      hello_world_method_window_document(hello_world_borrow_window(window));

  hello_world_own_html_element_t body;
  if (!hello_world_method_document_body(hello_world_borrow_document(document),
                                        &body)) {
    return;
  }
  ...
  hello_world_own_dom_token_list_t classes =
      hello_world_method_element_class_list(hello_world_borrow_element(heading));
  ...
  hello_world_method_element_text_content(hello_world_borrow_element(heading),
                                          &text);
